...
export default function DevRelAgent() {
  ...
  const runAgent = async () => {
    ...
    try {
      ...
      setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), type: "error", text: `Stream error: ${err.message}` }]);
    } finally {
      setIsRunning(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0d1117] text-[#c9d1d9] font-sans relative">
      ...
      <header className="flex items-center justify-between px-8 py-6 border-b border-[#30363d] bg-[#161b22] sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          <a href="#" className="flex items-center gap-2 text-sm font-medium hover:text-white transition-colors">
            <FaGithub className="h-5 w-5" />
            View Source
          </a>
          <button className="bg-[#238636] hover:bg-[#2ea043] text-white px-4 py-2 rounded-md font-medium text-sm transition-colors border border-[rgba(240,246,252,0.1)]">
            Install GitHub App
          </button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="relative">
          <div className="absolute -inset-1 bg-gradient-to-r from-[#238636] to-[#a371f7] rounded-2xl blur opacity-20" />
          <div className="relative bg-[#010409] border border-[#30363d] rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="p-6 font-mono text-sm h-[380px] overflow-y-auto">
              ...
              <div className="flex items-center gap-2 mt-4 animate-pulse text-[#8b949e]">
                <span>_</span>
              </div>
            </div>
          </div>
